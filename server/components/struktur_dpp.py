...
class StrukturDPPComponent(BaseStrukturComponent):
    ...
    def show_org_chart(self):
        ...
        try:
            ...
            # Kelompokkan data berdasarkan jabatan
            jabatan_groups = {}
            for data in self.struktur_data:
                jabatan = data.get('jabatan_utama', 'Lainnya')
                if jabatan not in jabatan_groups:
                    jabatan_groups[jabatan] = []
                jabatan_groups[jabatan].append(data)

            # Tampilkan per jabatan
            for jabatan in sorted(jabatan_groups.keys()):
                jabatan_frame = QFrame()
                jabatan_frame.setStyleSheet("""
                    QFrame {
                ...
                jabatan_layout = QVBoxLayout(jabatan_frame)

                # Header jabatan
                jabatan_header = QLabel(f"{jabatan}")
                jabatan_header.setAlignment(Qt.AlignCenter)
                jabatan_header.setStyleSheet("""
                    QLabel {
                ...
                jabatan_layout.addWidget(members_frame)
                scroll_layout.addWidget(jabatan_frame)
    ...
    def create_person_widget(self, person_data):
        ...
        nama_label = QLabel(nama_lengkap)

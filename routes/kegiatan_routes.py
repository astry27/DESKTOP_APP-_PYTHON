...
def get_my_kegiatan():
    ...
    try:
        ...
        """)
        has_user_id = cursor.fetchone()['col_exists'] > 0
        ...
        for row in result:
            ...
            # Convert date fields
            if row.get('tanggal_mulai') and hasattr(row['tanggal_mulai'], 'isoformat'):
                row['tanggal_mulai'] = row['tanggal_mulai'].isoformat()
            if row.get('tanggal_selesai') and hasattr(row['tanggal_selesai'], 'isoformat'):
...
def get_kegiatan():
    ...
    try:
        ...
        if start_date:
            conditions.append("k.tanggal_mulai >= %s")
            params.append(start_date)

        if end_date:
            conditions.append("k.tanggal_selesai <= %s")
        ...
        query += " ORDER BY k.tanggal_mulai DESC LIMIT %s OFFSET %s"
...
def add_kegiatan():
    ...
    try:
        ...
        tanggal_mulai = data.get('tanggal_mulai') or data.get('tanggal')
        tanggal_selesai = data.get('tanggal_selesai') or data.get('tanggal')
        waktu_mulai = data.get('waktu_mulai') or data.get('waktu')
        ...
        if has_user_id:
            query = """
            INSERT INTO kegiatan (nama_kegiatan, deskripsi, lokasi, tanggal_mulai,
...
def update_kegiatan(kegiatan_id):
    ...
    try:
        ...
        UPDATE kegiatan SET
            ...
            tanggal_mulai = %s, tanggal_selesai = %s, waktu_mulai = %s,
    ...
    try:
        ...
        for row in result:
            transformed_data.append({
                ...
                'tanggal': row.get('tanggal_mulai'),
                'waktu': row.get('waktu_mulai'),
